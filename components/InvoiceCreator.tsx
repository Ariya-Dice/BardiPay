...
      const qrCodeData = `ethereum:${recipient}?amount=${amountCrypto}&token=${token}&invoiceId=${invoiceId}`;
...
              onChange={(e) => handleAmountChange(e.target.value)}
              className={`${styles.currencySelect} ${amountError ? styles.inputError : ''}`}
...
            <QRCode value={result.qrCodeData} size={200} />
          </div>
        </section>
      )}
    </>
